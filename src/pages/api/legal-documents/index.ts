...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getLegalDocumentsPublic();
    }
  ...
  async function getLegalDocumentsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'legal_document');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.legal_document.count(countOptions as unknown),
      prisma.legal_document.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
