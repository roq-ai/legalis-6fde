...
import { RenamedcaseListPage as ShowCaseList } from 'pages/renamedcases';
...
function DashboardPage() {
  ...
  const renamedcaseStat = useSWR<PaginatedInterface<RenamedcaseInterface>>(
    () => `/renamedcases`,
    () => getRenamedcases({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Renamedcases',
      result: renamedcaseStat?.data?.totalCount,
      isLoading: renamedcaseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`renamedcases`} passHref legacyBehavior>
                  <Button
