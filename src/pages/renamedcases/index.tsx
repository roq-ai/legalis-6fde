...
type ColumnType = ColumnDef<RenamedcaseInterface, unknown>;

interface RenamedcaseListPageProps {
...
export function RenamedcaseListPage(props: RenamedcaseListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RenamedcaseInterface>>(
    () => `/renamedcases?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RenamedcaseInterface) => {
    if (hasAccess('renamedcase', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/renamedcases/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/renamedcases/view/${record.id}`} passHref legacyBehavior>
                <Button
